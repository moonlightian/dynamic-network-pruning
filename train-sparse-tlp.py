...
wdir = 'sparse-group-tlp' + os.sep  # weights dir
last = wdir + 'last.pt'
best = wdir + 'best.pt'
...
def train():
    ...
    def simulated_annealing_p(mapOld, mapNew, p,epochNow, epochsTotal=opt.epochs):
        """

        :param mapOld: map of epoch-5
        :param mapNew: map of epoch
        :param p:
        :return: p new
        """
        if epochNow < 6:
        ...
        T=epochsTotal/(epochNow+1)
        sumT=300*(math.log(epochsTotal+1)+0.5772156649-2.45)
        bias=0.3/sumT
        pNew=p+np.random.uniform(low=-0.0006-bias,high=0.0006-bias)*T
        if (0<=pNew and pNew<=1):
            if mapNew>mapOld:
                p=pNew
            else:
                Pchange=math.exp(-(mapOld-mapNew)*300/T)
                Punchange=np.random.uniform(low=0,high=1)
                if Punchange<Pchange:
                    p=pNew
        #print ("p in epoch: ",epochNow," is: ",p)
        return p
    def simulated_annealing_Sparse(bn_old, bn_new, prune_idx,size_list,sparse,epochNow, epochsTotal=opt.epochs):
        ...
        bias=0.3/sumT
        #C=len(sparse)
        sum_old,sum_new=float(torch.sum(bn_old,0)),float(torch.sum(bn_new,0))
        #print("sparse:",sum(sparse))#"new:",sum_new,"old:",sum_old,
        new_l=bn_new.tolist()
        old_l=bn_old.tolist()
        #print(new_l,bn_new)
        index = 0
        for idx, size in zip(prune_idx, size_list):
            ...
            sparse_idx=sparse[index:(index + size)]
            for i in range(size):
                sparse_new=sparse_idx[i]+np.random.uniform(low=-0.0006-bias,high=0.0006-bias)*T
                if (0.0001<=sparse_new and sparse_new<=0.007):
                    delt_f=(new_l[i]-old_l[i])/sum_old
                    if delt_f>0:#按照bn的gamma值的占比来判断是否更改该channel的sparse
                        sparse_idx[i]=sparse_new
                    else:
                        Pchange=math.exp((delt_f)*300/T)
                        Punchange=np.random.uniform(low=0,high=1)
                        if Punchange<Pchange:
                            sparse_idx[i]=sparse_new
            sparse[index:(index + size)]=sparse_idx
            index += size
        return sparse
    # # Plot lr schedule
    # y = []
    ...
    #sparse=[]
    size_list = [model.module_list[idx][1].weight.data.shape[0] for idx in prune_idx]
    sparse=[0.003]*sum(size_list)
    #print(len(sparse[item]) for item in range(len(sparse)))
    with open('size.txt','a') as f7:
        f7.write(str(size_list)+' ')
    for idx in prune_idx:
        ...
        #size=bn_module.weight.data.shape
        #print(size[0],idx)
        #sparse.append([0.003]*size_list[idx])
        # with open('size.txt','a') as f7:

        #     f7.write(str(size_list[idx])+' ')
    # Start training
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if opt.sr and opt.prune==1 and epoch > opt.epochs * 0.5:
                idx2mask = get_mask2(model, prune_idx, 0.85) #0.95
            # change end

            
            #SPARSE change every batch according to the bn percent:
            bn_weights=gather_bn_weights(model.module_list, prune_idx)
            #print(bn_weights)
            bn_list.append(bn_weights)#获取bn值存入记录
            
            if i>=1:
                sparse=simulated_annealing_Sparse(bn_list[-2],bn_list[-1],prune_idx,size_list,sparse,epoch,opt.epochs)
                #print(len(bn_list[-1]))
                with open('sparse.txt', 'a') as f:
                    f.write(str(sparse)+'\n')
                BNOptimizer.updateBN_Sparse_tlp_SA(pinTLp,sr_flag, model.module_list, sparse, prune_idx, epoch, idx2mask, opt)
            else:
                BNOptimizer.updateBN_Sparse_tlp_SA(pinTLp,sr_flag, model.module_list, sparse, prune_idx, epoch, idx2mask, opt)
            ...
            #更改bn方式
            #BNOptimizer.updateBN_Sparse_SA(sr_flag, model.module_list, sparse, prune_idx, epoch, idx2mask, opt)
        ...
        map_list.append(results[2])
        if epoch > 5 :
            pinTLp=simulated_annealing_p(map_list[epoch-5],map_list[epoch],pinTLp,epoch,opt.epochs)
            print("pinTLp is: ", pinTLp)

        map_list.append(results[2])
        with open(wdir+'sparse_epoch.txt', 'a') as f:
            f.write(str(sparse)+'\n')
        #print("TOTOAL sparse:",sum(sparse))
        with open(wdir+'sparse_total.txt', 'a') as f2:
            f2.write(str(sum(sparse))+'\n')
        
        # if epoch > 5 :
        #     sparse=simulated_annealing_Sparse(map_list[epoch-5],map_list[epoch],sparse,epoch,opt.epochs)
        #     print("sparse is: ", sparse)
        #     with open('sparse.txt','a') as f4:
        #         f4.write(str(sparse)+'\n')
        #print(num_no_increase_epoch)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--s', type=float, default=0.003, help='scale sparse rate')
